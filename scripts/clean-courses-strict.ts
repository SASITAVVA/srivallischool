...
async function cleanCoursesStrict() {
  const snapshot = await db.collection('courses').get();
  
  // The definitive 3 courses we want to keep
  const keepIds = ['eavKpuNSfJvkbIZUnUmi', 'yNUZHE09J11DDYlEBl53', 'demo-course-storytelling'];

  for (const doc of snapshot.docs) {
    if (!keepIds.includes(doc.id)) {
      console.log(`Deleting extra/duplicate course: ${doc.id} - ${doc.data().title || doc.data().name}`);
      await db.collection('courses').doc(doc.id).delete();
    } else {
      console.log(`Keeping definitive course: ${doc.id} - ${doc.data().title || doc.data().name}`);
    }
  }
}

cleanCoursesStrict().catch(console.error);
